...
import random
from tools.utils_cv import _pickle_keypoints
...
def get_args():
    ...
    """
    parser = argparse.ArgumentParser(description="Match DDAD SIFT features")
    parser.add_argument("--root_path", type=str, default='../data/ddad/sift', help="Root path to the SIFT data")
    parser.add_argument("--root_save_path", type=str, default='../data/ddad/match', help="Root path to save the match data")
    parser.add_argument("--mask_path", type=str, default='../data/ddad/mask', help="Path to the mask data")
    parser.add_argument("--info_path", type=str, default='../datasets/ddad/info_train.pkl', help="Path to the info file")
...
def compute_match(src_sift, tgt_sift, src_K, tgt_K, src_auto_mask, tgt_auto_mask, T_gt):
    """
    Compute matches between SIFT features of source and target images.
    """
    ...
    kp1_np = np.array([list(kp.pt) for kp in kp1_ori])
    kp2_np = np.array([list(kp.pt) for kp in kp2_ori])

    mask1 = (kp1_np[:, 0] > 0) & (kp1_np[:, 0] < WIDTH) & (kp1_np[:, 1] > 0) & (kp1_np[:, 1] < HEIGHT)
    mask2 = (kp2_np[:, 0] > 0) & (kp2_np[:, 0] < WIDTH) & (kp2_np[:, 1] > 0) & (kp2_np[:, 1] < HEIGHT)

    mask3 = src_auto_mask[kp1_np[:, 1].astype(int), kp1_np[:, 0].astype(int)] == 0
    mask4 = tgt_auto_mask[kp2_np[:, 1].astype(int), kp2_np[:, 0].astype(int)] == 0
    ...
    good_np = np.array([[m[0].queryIdx, m[0].trainIdx] for m in good])
    if len(good_np) == 0:
        return True, 0, 0
    kp1_np = np.array([kp.pt for kp in kp1])
    kp2_np = np.array([kp.pt for kp in kp2])
    ...
    F = np.linalg.inv(tgt_K.T) @ Tx @ T_gt[:3, :3] @ np.linalg.inv(src_K)
    ...
    cam_points_0 = np.dot(src_K_invert, cam_points_0.T)
    cam_points_1 = np.concatenate([kp_ransac_np[:, 1, :], np.ones([kp_ransac_np.shape[0], 1])], -1)
    cam_points_1 = np.dot(tgt_K_invert, cam_points_1.T)
    depth_0, angle_0 = compute_depth(cam_points_0.T, (np.dot(T_invert[:3, :3], cam_points_1)).T, T_invert[:3, 3])
    depth_1, angle_1 = compute_depth(cam_points_1.T, (np.dot(T_temp[:3, :3], cam_points_0)).T, T_temp[:3, 3])
...
def compute_depth(point_0, point_1, point_ori):
    """
    Compute depth and angle between points.
    """
    x0, y0, z0 = 0, 0, 0
    cos_0 = point_0.copy()
    cos_0_norm = np.linalg.norm(cos_0, axis=-1)
    cos_1 = point_1.copy()
    cos_1_norm = np.linalg.norm(cos_1, axis=-1)
    ...
    k0 = (y0 - point_ori[1]) * (b0 * c1 - c0 * b1) - (x0 - point_ori[0]) * (c0 * a1 - a0 * c1)
    k1 = (z0 - point_ori[2]) * (b0 * c1 - c0 * b1) - (x0 - point_ori[0]) * (a0 * b1 - b0 * a1)
...
def process(args):
    """
    Process each frame to compute matches and save results.
    """
    source_id, info, root_path, root_save_path, mask_path, camera_names = args
    ...
    for camera_id, camera_name in enumerate(camera_names):
        ...
        save_path = os.path.join(root_save_path, scene_name, 'match', camera_name, source_id + '.pkl')
    ...
    for camera_id in range(len(camera_names)):
        ...
        flag, depth1, depth2 = compute_match(sifts[camera_id], sifts[(camera_id + 1) % 6],
                                             info[source_id][camera_names[camera_id]]['intrinsics'],
                                             info[source_id][camera_names[(camera_id + 1) % 6]]['intrinsics'],
...
def main():
    ...
    pool_args = [(source_id, info, root_path, root_save_path, mask_path, camera_names) for source_id in info_list]

    with multiprocessing.Pool(num_workers) as pool:
        for _ in tqdm(pool.imap_unordered(process, pool_args), total=len(pool_args)):
            pass
