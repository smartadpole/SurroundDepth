...
class DDADDataset(MonoDataset):
    ...
    def get_info(self, inputs, index_temporal, do_flip):
        ...
        for index_spatial in range(6):
            file_rgb = os.path.join(self.rgb_path, scene_id, 'rgb', self.camera_names[index_spatial], index_temporal + '.png')
            try:
                inputs['id'].append(self.camera_ids[index_spatial])
                color = self.loader(file_rgb)
                ...
                print(f"Error processing file: {file_rgb}, Error: {e}")
                return False
