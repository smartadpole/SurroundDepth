...
def vec_from_rot(rot):
    trans = rot[:,:3,3]/torch.norm(rot[:,:3,3], p=2, dim=-1)
    temp = (rot[:,:3,:3] - rot[:,:3,:3].permute(0,2,1))/2
    angle_vec = torch.stack([temp[:,2,1],-temp[:,2,0],temp[:,1,0]], dim=1)
    angle = torch.norm(angle_vec,p=2,dim=-1)
    axis = angle_vec/angle[:,None]
    return torch.asin(angle)[:,None]*axis, trans
